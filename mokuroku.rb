...
# 【要設定項目】
# src_dir: XYZタイルが置かれたディレクトリ
# tes: タイル種別を示すフォルダ名とタイルファイルの拡張子の配列の配列

# オリジンサーバで本スクリプトを実行する。
# ココタイル作成に必要な目録情報が標準出力に出力されるので、
# ファイルにリダイレクトして後段の CSV ファイル変換
# ( gsi-cyberjapan/cocotile-mokuroku2csv ) を実行する。
